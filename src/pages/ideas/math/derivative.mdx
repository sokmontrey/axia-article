...
We still use a second point for the formula, _but what if it is very close to the first one?_ **Not on top, very close**. Let's rewrite the formula. Instead of using another point, $x_2$, we take the input, $x$, and add a very small number to it. Call this small number $\Delta x$. Now, here is how we will find the rate of change:
...
Now, we can plug in `h=0` and get $2x$. So the derivative of $f(x) = x^2$ with respect to $x$ is:
...
Remember, h is very small, but not zero. This mean that the two points are very close to each other, but not on top of each other.

> <span class="title">Additional Info</span>
> [Derivative rules](/ideas/math/derivative-rules) are built on top of the limit definition of derivative. 
